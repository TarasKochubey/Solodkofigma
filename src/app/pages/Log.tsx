...
const LogMealCard = ({ entry, index, reduceMotion }: any) => (
  <motion.div
    initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 14 }}
    animate={reduceMotion ? { opacity: 1 } : { opacity: 1, y: 0 }}
    transition={{ delay: reduceMotion ? 0 : 0.08 + index * 0.04, duration: reduceMotion ? 0.2 : 0.5, ease: "easeOut" }}
    className="flex gap-4 items-start relative z-10"
  >
    <div className="w-[36px] shrink-0 font-sans text-[12px] font-medium text-[#8B918C] pt-5 text-right">
      {entry.time}
    </div>
    <div className="w-2.5 h-2.5 rounded-full bg-[#FFF8F0] border-[2px] border-[#DCEEDC] shadow-[0_0_0_5px_rgba(255,248,240,1)] mt-[21px] shrink-0 relative z-20" />
    <div className="flex-1 bg-gradient-to-br from-white/68 to-white/38 p-4.5 rounded-[24px] shadow-[0_10px_28px_-16px_rgba(180,140,120,0.18)] border border-white/58">
      <div className="flex justify-between items-start gap-3">
        ...
        <div className="text-right shrink-0">
          <span className="font-sans text-[22px] leading-none font-semibold text-[#1F2422] tracking-tight">
...
export const Log = () => {
  ...
  return (
    <>
      ...
      >
        <button onClick={() => navigate(-1)} aria-label="Back" className="w-11 h-11 flex items-center justify-center text-[#5F6661] bg-white/30 backdrop-blur-md rounded-full hover:bg-white/50 transition-colors">
      ...
      <div className="px-6 mt-2 pb-36 relative">
        {logs.length > 0 && (
          <div className="absolute left-[45px] top-10 bottom-0 w-[2px] bg-gradient-to-b from-[#1F2422]/6 via-[#1F2422]/4 to-transparent z-0 rounded-full" />
        )}
          ...
          groupedLogs.map(({ section, entries }) => entries.length > 0 && (
            <section key={section} className="mb-9">
              ...
              <div className="space-y-4.5">
                {entries.map((entry, i) => (
                  <LogMealCard key={entry.id} entry={entry} index={i} reduceMotion={reduceMotion} />
                ))}
              </div>
            </section>
          ))
