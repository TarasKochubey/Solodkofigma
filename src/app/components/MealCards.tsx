...
import { Check, Info, Clock, Loader2, Sparkles, Image as ImageIcon } from 'lucide-react';
import { motion } from 'motion/react';

export const RecurringCard = ({ title, carbs, weight, time, delay, matchScore, onClick }: { title: string; carbs: number; weight: string; time: string; delay: number; matchScore?: number; onClick?: () => void }) => (
  <motion.div 
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="font-sans text-[13px] leading-[18px] text-[#8B918C] mt-0.5">
            {matchScore ? <span className="text-[#A3D9A5] mr-1">{matchScore}% match</span> : null}
            {weight}
          </p>
        </div>
      </div>
...
export const ExpandedMealCard = ({ title, carbs, details, source, isEstimated = false, confidence, lastEnjoyed, onLog, onCancel }: any) => (
  <div className="p-6 rounded-[32px] bg-white/60 backdrop-blur-[48px] shadow-[0_32px_64px_-16px_rgba(200,160,140,0.25)] relative overflow-hidden group border border-white/70">
    <div className="absolute inset-0 shadow-[inset_0_2px_12px_rgba(255,255,255,0.9)] pointer-events-none rounded-[32px]" />
    
    {/* Subtle atmospheric glow in the background of the card */}
      ...
      <div className="pr-4">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          <span className="font-sans text-[11px] leading-none text-[#5F6661] font-semibold uppercase tracking-[0.15em] bg-white/80 px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
            {source === 'AI' && <Sparkles size={10} />}
            {source}
          </span>
          {confidence && (
            <span className="font-sans text-[11px] leading-none text-[#A3D9A5] font-semibold uppercase tracking-[0.1em] bg-white/80 px-2 py-1 rounded-full shadow-sm border border-[#A3D9A5]/30">
              {confidence}% Confidence
            </span>
          )}
          {isEstimated && <span className="font-sans text-[13px] text-[#8B918C]">Estimated</span>}
        </div>
        <h2 className="font-sans text-[24px] leading-[28px] font-medium text-[#1F2422] tracking-tight whitespace-pre-line">{title}</h2>
      </div>
      <div className="text-right shrink-0">
        <div className="font-sans text-[56px] leading-[0.8] font-semibold text-[#1F2422] tracking-tighter flex items-baseline">
          {carbs}<span className="text-[20px] text-[#5F6661] font-medium ml-1">g</span>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-1 mb-6 relative z-10">
      <div className="flex items-baseline justify-between">
        <span className="font-sans text-[16px] leading-snug font-medium text-[#1F2422]">{details.portion}</span>
        {lastEnjoyed && (
          <span className="font-sans text-[12px] leading-snug text-[#8B918C]">Last: {lastEnjoyed}</span>
        )}
      </div>
      <span className="font-sans text-[14px] leading-snug text-[#8B918C]">{details.ratio}</span>
    </div>
    ...
    <div className="flex flex-col gap-2 relative z-10">
      ...
      >
        <Check size={18} strokeWidth={2.5} className="text-[#A3D9A5]" />
        Log this meal
      </button>
      <div className="flex gap-2">
        <button className="flex-1 bg-white/40 backdrop-blur-md rounded-[20px] py-3 font-sans text-[14px] font-medium text-[#5F6661] hover:bg-white/60 transition-colors border border-white/40">
          Adjust portion
        </button>
        <button onClick={onCancel} className="flex-1 bg-white/40 backdrop-blur-md rounded-[20px] py-3 font-sans text-[14px] font-medium text-[#5F6661] hover:bg-white/60 transition-colors border border-white/40">
          Dismiss
        </button>
      </div>
    </div>
